...
Mac worker → Supabase 폴링 (1초마다)
           ...
           → workspace/ingest 아래로 yt-dlp 최대 1080p 다운로드
           → 로컬 workspace source 보존
...
| 파일 | 역할 |
|---|---|
| `worker.py` | Supabase 폴링 루프 — yt-dlp + 로컬 workspace source 보존 |
...
## Desktop 경로에서 detached 실행

이 레포가 `~/Desktop/...` 아래에 있으면 macOS privacy control 때문에
LaunchAgent가 `Operation not permitted`로 실패할 수 있다. 이 경우 현재
사용자 세션 권한을 상속하는 detached `screen` runner를 사용한다.

```bash
workers/ingest/start-screen.sh
tail -f /tmp/galaxymap-ingest.log
tail -f /tmp/galaxymap-ingest-error.log

# 중지
workers/ingest/stop-screen.sh
```
...
| `SUPABASE_SERVICE_ROLE_KEY` | RLS를 우회하는 로컬 worker 전용 키 |
| `STORAGE_ROOT` | 로컬 workspace 루트. 기본값 `<repo>/workspace`; ingest source는 `<repo>/workspace/ingest/...` |
| `YTDLP_CONCURRENT_FRAGMENTS` | yt-dlp 병렬 fragment 수. 기본값 `8`; 품질을 낮추지 않고 다운로드 시간을 줄이는 설정 |
| `ARIA2_CONNECTIONS` | aria2 연결 수. 기본값은 `YTDLP_CONCURRENT_FRAGMENTS`와 동일 |
| `YTDLP_BIN` | 선택. launchd PATH 밖에 yt-dlp가 있을 때 absolute path 지정 |
| `FFPROBE_BIN` | 선택. launchd PATH 밖에 ffprobe가 있을 때 absolute path 지정 |
| `ARIA2C_BIN` | 선택. aria2c absolute path 지정. 없으면 aria2 없이 진행 |

R2 source storage is temporarily disabled. `yt_source_path` remains a UID-prefixed
relative key, but the bytes live at `workspace/ingest/{yt_source_path}` and are
served through the Next.js server route.
